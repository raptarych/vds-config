...
Для бэкапа всех конфигов (например для переноса на другой VDS) используйте:
...
Для восстановления достаточно передать ссылку на `tar`-файл:

```bash
bash restore.sh "<ссылка_на_backup.tar>"
```